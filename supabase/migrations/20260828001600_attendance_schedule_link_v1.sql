...
returns table (
  ...
  store_id uuid, store_code text, store_name text, schedule_status text,
  attendance_id uuid, attendance_status text, check_in timestamptz, check_out timestamptz,
  planned_start time, planned_end time, late_minutes integer, early_minutes integer,
  hours_worked numeric, amount numeric, grade text, hourly_rate numeric
)
language sql security definer stable set search_path = public
as $$
  select ws.id, ws.work_date, ws.start_time, ws.end_time, ws.store_id, s.code, s.name, ws.status,
         a.id, a.status, a.check_in, a.check_out, a.planned_start, a.planned_end,
         a.late_minutes, a.early_minutes, a.hours_worked, a.amount, a.grade, a.hourly_rate
    from public.work_schedules ws
    join public.stores s on s.id = ws.store_id
    left join public.attendance a on a.schedule_id = ws.id
      and a.status not in ('DELETED','DELETED_BY_MANAGER')
   where ws.user_id = auth.uid()
     and ws.status = 'APPROVED'
     and ws.work_date = (now() at time zone 'Asia/Ho_Chi_Minh')::date
...
begin
  ...
  select * into v_schedule from public.work_schedules
   where id=p_schedule_id and user_id=auth.uid() and status='APPROVED' for share;
  ...
  if exists(select 1 from public.attendance a where a.user_id=auth.uid() and a.status='OPEN') then
    raise exception 'OPEN_ATTENDANCE_EXISTS';
  end if;
  if exists(select 1 from public.attendance a where a.schedule_id=p_schedule_id and a.status not in ('DELETED','DELETED_BY_MANAGER')) then
    raise exception 'SCHEDULE_ALREADY_ATTENDED';
  end if;
  if not exists(select 1 from public.profiles p where p.id=auth.uid() and p.status='ACTIVE') then
    raise exception 'EMPLOYEE_NOT_ACTIVE';
  end if;
  select * into v_grade from public.employee_grades where user_id=auth.uid() and status='ACTIVE' limit 1;
  ...
  insert into public.attendance (
    ...
    v_schedule.work_date,auth.uid(),v_schedule.store_id,v_schedule.id,now(),'OPEN',
    v_schedule.start_time,v_schedule.end_time,v_grade.grade,v_grade.hourly_rate,
    greatest(0,floor(extract(epoch from (v_local_check_in-v_planned_start))/60))::integer,0,0,0
...
begin
  if auth.uid() is null then raise exception 'AUTH_REQUIRED'; end if;
  select * into v_att from public.attendance where id=p_attendance_id and user_id=auth.uid() and status='OPEN' for update;
  ...
  update public.attendance set check_out=now(),status='COMPLETED',
    early_minutes=greatest(0,floor(extract(epoch from (v_planned_end-v_local_checkout))/60))::integer,
    hours_worked=v_hours,amount=v_amount
...
create or replace function public.get_my_attendance_v2(p_from_date date default null,p_to_date date default null)
returns table (
  attendance_id uuid,schedule_id uuid,work_date date,store_id uuid,store_code text,store_name text,
  planned_start time,planned_end time,check_in timestamptz,check_out timestamptz,status text,
  late_minutes integer,early_minutes integer,hours_worked numeric,amount numeric,grade text,hourly_rate numeric
)
language sql security definer stable set search_path = public
as $$
  select a.id,a.schedule_id,a.work_date,a.store_id,s.code,s.name,a.planned_start,a.planned_end,
         a.check_in,a.check_out,a.status,a.late_minutes,a.early_minutes,a.hours_worked,a.amount,a.grade,a.hourly_rate
    from public.attendance a join public.stores s on s.id=a.store_id
