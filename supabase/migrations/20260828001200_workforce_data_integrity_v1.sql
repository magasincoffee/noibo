...
-- 3. Preserve updated_at semantics for all Workforce V2 mutable entities.
...
-- Generation assignments are mutable during review/revalidation.
-- The table currently has no updated_at column, so no updated-at trigger is
-- added there; review mutations are represented by assignment status/note and
-- generation lifecycle instead.
